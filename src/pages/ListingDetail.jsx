      ...
      <div className="md:flex md:gap-10">
        <div className="md:w-1/2 md:shrink-0">
          <div className="relative mx-3 md:mx-0 rounded-xl overflow-hidden bg-neutral-100 dark:bg-neutral-800">
            ...
            {grade.letter === 'A' && (
              <span className="absolute top-2 left-2 bg-white/90 dark:bg-neutral-900/90 rounded-full p-1.5 flex items-center gap-1 px-2" title="Verified great condition">
                <ShieldCheck size={14} className="text-emerald-600" />
                <span className="text-[11px] font-medium text-emerald-700 dark:text-emerald-400">Verified</span>
              </span>
            )}
            {photoDefects.map((d) => (
              <div
        ...
        <div className="px-3 md:px-0 mt-3 md:mt-0 md:w-1/2 space-y-3">
          <div>
            ...
            <p className="text-sm text-neutral-400">Sold by {listing.seller}</p>
          </div>

          <span className="inline-flex items-center gap-1 bg-emerald-600 text-white text-xs font-semibold rounded px-1.5 py-0.5">
          ...
          <p className="text-xs text-neutral-400 flex items-center gap-1.5">
            Sold by {listing.seller} · {stats.itemsSold} items sold · usually ships in {stats.shipsInDays}d
            {points > 0 && (
          ...
          <div>
            ...
            {showReasoning && (
              <div className="rounded-xl bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 p-3 space-y-2 text-sm mt-2">
                ...
                {grade.questionnaireBreakdown.map((q) => (
                  <p key={q.key} className="text-neutral-700 dark:text-neutral-300">
                    {q.key}: <span className="font-medium">{q.value}</span>
                    {q.penalty > 0 && <span className="text-neutral-400"> (-{q.penalty})</span>}
                  </p>
                ))}
                {grade.provenanceBreakdown?.length > 0 && (
                  <>
                    <p className="text-xs font-semibold text-neutral-400 uppercase tracking-wide pt-1">Trust &amp; provenance</p>
                    {grade.provenanceBreakdown.map((p) => (
                      <p key={p.key} className="text-neutral-700 dark:text-neutral-300">
                        {p.key}: <span className="font-medium">{String(p.value)}</span>
                        {p.penalty > 0 && <span className="text-neutral-400"> (-{p.penalty})</span>}
                      </p>
                    ))}
                  </>
                )}
                <p className="text-xs font-semibold text-neutral-400 uppercase tracking-wide pt-1">AI-detected defects ({grade.source})</p>
                {grade.defectBreakdown.length === 0 && <p className="text-neutral-400">No defects detected.</p>}
                {grade.defectBreakdown.map((d) => (
                  <div key={d.id} className="flex justify-between items-start">
                    <div>
                      <p className="text-neutral-700 dark:text-neutral-300">{d.label}</p>
                      <p className="text-xs text-neutral-400">
                        {d.severity} · confidence {Math.round(d.confidence * 100)}% · photo {d.imageIndex + 1}
                      </p>
                    </div>
                    <span className="text-neutral-400 text-xs">-{d.penalty}</span>
                  </div>
                ))}
              </div>
            )}
